...
void slack_react_get_reactions(sa, obj, args[0]) {
}
void slack_react_post_to_timestamp(sa, obj, args[0]) {
}
...
static void slack_react_lookup_ts(SlackAccount *sa, slack_thread_lookup_ts_cb *cb, SlackObject *conv, gpointer data, const char *timestr) {
	char start[20] = "";
	char end[20] = "";
	const char *rest;
	if (!slack_parse_thread_time(sa, timestr, start, end, &rest)) {
		slack_write_message(sa, conv, "Could not parse message timestamp.", PURPLE_MESSAGE_SYSTEM);
	...
	slack_api_get(sa, slack_thread_lookup_ts_history_cb, lookup, "reactions.get", "channel", id, "oldest", start, "latest", end, "inclusive", "1", NULL);
}
...
void slack_react_post_to_timestamp(SlackAccount *sa, SlackObject *obj, const char *timestr_and_msg) {
    slack_react_lookup_ts(sa, slack_post_post_lookup_cb, obj, NULL, timestr_and_msg);
}
...
void slack_react_get_reactions(SlackAccount *sa, SlackObject *obj, const char *timestr) {
}
